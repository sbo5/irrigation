...
def ode(x, t, u, p):
    ...
    for i in range(0, numberOfNodes):
        # print('time: ', timeList_downsample)
        # print('nodes: ', i)
...
def simulate(p):
    h = zeros((len(timeList_downsample), numberOfNodes))
    h[0] = hMatrix
    h0 = h[0]
    theta = zeros((len(timeList_downsample), numberOfNodes))
    h_avg = zeros((len(timeList_downsample), 4))  # 4 sensors
    h_avg[0] = hIni
    theta_avg = zeros((len(timeList_downsample), 4))
    theta_avg[0] = thetaIni*100
    for i in range(len(timeList_downsample)-1):
        # print('At time:', i+1, ' min(s)')
        ts = [timeList_downsample[i], timeList_downsample[i+1]]
...
def objective(p):
    ...
    # # Calculate objective
    obj = 0.0
    for i in range(len(timeList_downsample)):
        if obj_fun == 1:
...
numberOfSamples = 10
sampleInterval = int(10/numberOfSamples)
...
for i in range(numberOfSamples):
    ...
    timeList_downsample.append(timeList[i*sampleInterval])
timeList_downsample = array(timeList_downsample, dtype='O')

irr_amount = zeros((len(timeList), 1))
for i in range(0, interval):
    if i in range(0, 22):
        irr_amount[i] = (0.001 / (3.1415 * 0.22 * 0.22) / (22 * 60))
...
# bnds = ((1e-10, 1e-2), (0.0, 1.0), (0.0, 1.0), (-inf, inf), (-inf, inf))
# solution = optimize.minimize(objective, p0, method='L-BFGS-B', bounds=bnds)
# p = solution.x
...
# hp = simulate(p)
#
# theta_i = 100*((p0[1]-p0[2])*(1 + (-Alpha*hi)**p0[4])**(-(1-1/p0[4])) + p0[2])
# theta_p = 100*((Theta_s-Theta_r)*(1 + (-Alpha*hp)**N)**(-(1-1/N)) + Theta_r)
#
# plt.figure(1)
# # plt.subplot(4, 1, 1)
...
#
# # io.savemat('sim_results.mat', dict(y=solList_thetaAvg))
