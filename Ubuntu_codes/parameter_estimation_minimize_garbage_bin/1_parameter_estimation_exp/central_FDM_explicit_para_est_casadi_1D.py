...
def capillary_capacity(h, theta_s, theta_r, alpha, n, s=S):
    cc = 0.5*(((1+np.sign(h))*s)+(1-np.sign(h))*(s+((theta_s*Theta_s_s-theta_r*Theta_r_s)*alpha*Alpha_s*n*N_s*
        (1-1/(n*N_s)))*((-1*(alpha*Alpha_s)-1*((h)**2)**(0.5))**((n*N_s)-1))*((1+(-1*(alpha*Alpha_s)-1*((h)**2)**(0.5))
        **(n*N_s))**(-(2-1/(n*N_s))))))
    return cc
