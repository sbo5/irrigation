...
def hydraulic_conductivity(h, ks=Ks, alpha=Alpha, n=N):
    hc = 0.5*(((1+np.sign(h))*ks)+(1-np.sign(h))*ks*(((1+((-1*alpha*-1*(h**2)**(1./2.))**n))**(-(1-1/n)))**(1/2))*
              ((1-(1-((1+((-1*alpha*-1*(h**2)**(1./2.))**n))**(-(1-1/n)))**(n/(n-1)))**(1-1/n))**2))
    return hc
...
def psiTopFun(hTop0, psi, qTop, dz):
    F = psi + 0.5*dz*(-1-(qTop+max(0, hTop0/dt))/hydraulic_conductivity(hTop0)) - hTop0  # Switching BC
...
def simulate(p):
    ...
    # Boundary conditions
    qTfun = irrigation
    for t in range(0, len(timeList)-1):
        print('From', t, ' min(s), to ', t + 1, ' min(s)')
        ts = [timeList[t], timeList[t + 1]]

        y = integrate.odeint(ode, h0, ts, args=(qTfun[t],qTfun[t]))
        h0 = y[-1]

        theta0 = thetaFun(h0, p)
        theta0 = theta0[::-1]
        theta[t+1] = theta0
        theta_avg[t+1] = np.matmul(CMatrix, theta0)

        x1 = h0[::-1].copy()
        h[t+1] = x1
    return h, theta_avg, theta, h
...
# Inputs: irrigation
# ----------------------------------------------------------------------------------------------------------------------
# The idea here is to irrigate the water constantly
...
# Main
# ----------------------------------------------------------------------------------------------------------------------
h_i, theta_i, theta_i_all, h_i_all = simulate(p=Loam())
