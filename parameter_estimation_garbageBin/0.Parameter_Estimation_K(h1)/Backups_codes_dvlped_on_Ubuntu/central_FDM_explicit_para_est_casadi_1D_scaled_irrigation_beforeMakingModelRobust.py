...
# thetaIni = array([26.2,	12.5, 9.6, 10.7])/100
# Original
thetaIni = array([30.2, 8.8, 8.7, 10.0])/100

hIni = zeros(4)
for i in range(0, 4):
    hIni[i] = ((((thetaIni[i] - Theta_r_s)/(Theta_s_s-Theta_r_s))**(1./(-(1-1/N_s))) - 1)**(1./N_s))/(-Alpha_s)
...
def hydraulic_conductivity(h, ks, alpha, n):
    term3 = (1+((-1*(alpha*Alpha_s)*-1*(h**2)**(1./2.))**(n*N_s)))
    term4 = (term3**(-(1-1/(n*N_s))))
    term5 = term4**(1./2.)
    term6 = term4**((n*N_s)/((n*N_s)-1))
    term7 = (1-term6)**(1-1/(n*N_s))
    term8 = ((1-term7)**2)
...
def capillary_capacity(h, theta_s, theta_r, alpha, n, s=S):
    cc = 0.5*(((1+np.sign(h))*s)+
              (1-np.sign(h))*(s+((theta_s*Theta_s_s-theta_r*Theta_r_s)*(alpha*Alpha_s)*(n*N_s)*(1-1/(n*N_s)))*((-1*(alpha*Alpha_s)*-1*((h)**2)**(0.5))**((n*N_s)-1))*
                              ((1+(-1*(alpha*Alpha_s)*-1*((h)**2)**(0.5))**(n*N_s))**(-(2-1/(n*N_s))))))
    return cc
...
def objective(x, p, theta_e):
    ...
    temp11 = (1 + (-(alpha*Alpha_s) * x) ** (n*N_s))
    temp22 = temp11 ** (-(1 - 1. / (n*N_s)))
...
plb = array([Ks, Theta_s*0.9, Theta_r, Alpha, N])
pub = array([Ks, Theta_s*1.1, Theta_r, Alpha, N])
# plb = array([Ks*0.6, 0.45/Theta_s_s, 0.075/Theta_r_s, 3.5/Alpha_s, 1.4/N_s])
# pub = array([Ks, 0.5/Theta_s_s, 0.084/Theta_r_s, 4.0/Alpha_s, 1.7/N_s])
...
# with open('Data/exp_data_irrigation_only.dat', 'r') as f:
with open('Data/exp_data_original.dat', 'r') as f:
    whole = f.readlines()
    for index, line in enumerate(whole):
        ...
        for index1, item in enumerate(one_line):
            ...
            temp1 = ((item/100 - Theta_r*Theta_r_s) / (Theta_s*Theta_s_s - Theta_r*Theta_r_s))
            temp2 = (temp1 ** (1. / (-(1. - (1. / (N*N_s))))))
            temp3 = (temp2 - 1)
            temp4 = (temp3 ** (1. / (N*N_s)))
            item = temp4 / (-Alpha*Alpha_s)
            h_temp.append(item)
...
for i in range(0, interval):
    if i in range(1151*ratio_t, 1217*ratio_t):
        irr_amount[i] = (0.004 / (pi * 0.22 * 0.22) / ((1216-1151)*ratio_t * 60))
    else:
        irr_amount[i] = 0
