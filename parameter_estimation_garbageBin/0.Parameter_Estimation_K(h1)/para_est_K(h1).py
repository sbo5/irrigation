...
# # This is the results obtained from 10% bounds for initial guess (Loam), 2683
Ks_s = 3.65444585e-06  # [m/s]
Theta_s_s = 3.94909738e-01
Theta_r_s = 7.82718194e-02
Alpha_s = 2.75400000e+00
N_s = 1.56184344e+00
...
def ini_state_32(p):
    hMatrix = ((((((thetaMatrix/100 - p[2] * Theta_r_s) / (p[1] * Theta_s_s - p[2] * Theta_r_s + mini)) + mini) ** (
            1. / (-(1 - 1 / (p[4] * N_s + mini)) + mini)) - 1) + mini) ** (1. / (p[4] * N_s + mini))) / (
                   -p[3] * Alpha_s + mini)
    return hMatrix
...
# plb = array([Ks, Theta_s, Theta_r, Alpha*0.65, N*0.65])
# pub = array([Ks, Theta_s, Theta_r, Alpha*1.35, N*1.35])
plb = array([Ks*0.9, 0.4/Theta_s_s, Theta_r*0.9, Alpha*0.9, N*0.9])
pub = array([Ks*1.1, Theta_s*1.1, 0.083/Theta_r_s, Alpha*1.1, N*1.1])
thetaMatrix = thetaFun(h0, p0)
...
for i in range(interval):
    if i == 0:
        G = ini_state_32(K)  # Initial state
    else:
        pass
