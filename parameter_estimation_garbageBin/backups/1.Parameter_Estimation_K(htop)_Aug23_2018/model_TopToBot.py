...
def RichardsEQN_1D(psi, t, p, qTfun, qBot, psiTop, psiBot, qEva, qPrec, qRoot, qDrain, hAtm, hPond, dz, tIni, theta0):
...
def simulate(p):
    ...
    for i in range(len(timeList)-1):  # in ts, end point is timeList[i+1], which is 2682*60
        ...
        y = integrate.odeint(RichardsEQN_1D, h0, ts, args=(p, qTfun[i], qBot, psiTop, psiBot, qEva, qPrec, qRoot, qDrain, hAtm, hPond, dz, ts[0], theta0))
        h0 = y[-1]
        h[i + 1] = h0
