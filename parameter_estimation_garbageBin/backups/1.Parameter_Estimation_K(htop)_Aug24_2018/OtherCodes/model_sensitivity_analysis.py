...
def RichardsEQN_1D(psi, t, p, qTfun, qBot, psiTop, psiBot, qEva, qPrec, qRoot, qDrain, hAtm, hPond, dz, dt, theta0):
    ...
    # Top boundary
    qIrr = qTfun
    qTop = abs(qEva) - abs(qPrec) - abs(qIrr) - abs(hPond)/dt  # Potential flux at the soil surface
...
h_i_U, theta_i_U, theta_i_all_U, h_i_all_U = simulate(pars_U)
h_i_L, theta_i_L, theta_i_all_L, h_i_all_L = simulate(pars_L)

percentageDiff_U = np.sum(abs(theta_i_U-theta_i)/theta_i, axis = 0)/interval*100
percentageDiff_L = np.sum(abs(theta_i_L-theta_i)/theta_i, axis = 0)/interval*100

print('Percentage difference between reference and larger parameter is: ', percentageDiff_U, ' %')
print('Percentage difference between reference and smaller parameter is: ', percentageDiff_L, ' %')

plt.figure()
plt.plot(timeList/60.0, theta_i[:, 0], 'y-', label=r'$theta_1$ reference')
plt.plot(timeList/60.0, theta_i_U[:, 0], 'b:', label=r'$theta_1$ +20%')
plt.plot(timeList/60.0, theta_i_L[:, 0], 'r:', label=r'$theta_1$ -6%')
plt.plot(timeList/60.0, theta_e[:, 0], 'g-', label=r'$theta_1$ experimental')
...
plt.figure()
plt.plot(timeList/60.0, theta_i[:, 1], 'y-', label=r'$theta_2$ reference')
plt.plot(timeList/60.0, theta_i_U[:, 1], 'b:', label=r'$theta_2$ +20%')
plt.plot(timeList/60.0, theta_i_L[:, 1], 'r:', label=r'$theta_2$ -6%')
plt.plot(timeList/60.0, theta_e[:, 1], 'g-', label=r'$theta_2$ experimental')
...
plt.figure()
plt.plot(timeList/60.0, theta_i[:, 2], 'y-', label=r'$theta_3$ reference')
plt.plot(timeList/60.0, theta_i_U[:, 2], 'b:', label=r'$theta_3$ +20%')
plt.plot(timeList/60.0, theta_i_L[:, 2], 'r:', label=r'$theta_3$ -6%')
plt.plot(timeList/60.0, theta_e[:, 2], 'g-', label=r'$theta_3$ experimental')
...
plt.figure()
plt.plot(timeList/60.0, theta_i[:, 3], 'y-', label=r'$theta_4$ reference')
plt.plot(timeList/60.0, theta_i_U[:, 3], 'b:', label=r'$theta_4$ +20%')
plt.plot(timeList/60.0, theta_i_L[:, 3], 'r:', label=r'$theta_4$ -6%')
plt.plot(timeList/60.0, theta_e[:, 3], 'g-', label=r'$theta_4$ experimental')
