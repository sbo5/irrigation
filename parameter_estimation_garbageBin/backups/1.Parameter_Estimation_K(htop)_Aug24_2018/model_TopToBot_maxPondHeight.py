...
def psiTopFun(hTop0, p, psi, qTop, hPondMax, dz):
    F = psi[0] - 0.5*dz*((qTop+max(0, min(hTop0, hPondMax)))/KFun(hTop0, p)+1) - hTop0
    return F
...
def RichardsEQN_1D(psi, t, p, qTfun, qBot, psiTop, psiBot, qEva, qPrec, qRoot, qDrain, hAtm, hPond, hPondMax, dz, tIni, theta0):
    psiTop = optimize.fsolve(psiTopFun, psi[0], args=(p, psi, qTfun, hPondMax, dz))  # Initial guess psiTop = psi[0]
    if psiTop <= 0:
        ...
        print('Runoffffffffffffffffffffffffffffffffffffffffffffffffffffff')
        psiTop = psiTop  # Useless. But it helps to understand
        # hPond = 1e-06
        hPond = hPondMax
    ...
    qIrr = qTfun
    # dt = t-tIni+mini  # Cannot figure out when dt = 0
    ...
    # psiTop = optimize.fsolve(psiTopFun, psi[0], args=(p, psi, qTop, dz))  # Initial guess psiTop = psi[0]
...
def simulate(p):
    ...
    for i in range(len(timeList)-1):  # in ts, end point is timeList[i+1], which is 2682*60
        ...
        y = integrate.odeint(RichardsEQN_1D, h0, ts, args=(p, qTfun[i], qBot, psiTop, psiBot, qEva, qPrec, qRoot, qDrain, hAtm, hPond, hPondMax, dz, ts[0], theta0))
        h0 = y[-1]
        h[i + 1] = h0
