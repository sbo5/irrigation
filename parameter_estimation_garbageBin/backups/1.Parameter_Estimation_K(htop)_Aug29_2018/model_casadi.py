...
def psiTopFun(hTop0, p, psi, qTop, dz):
    F = psi[0] - 0.5*dz*((qTop+max(0, hTop0))/KFun(hTop0, p, pars)+1) - hTop0
    return F
...
def getALG(x, z, u, p, h):
    xTop = z[0]
    xBot = z[1]
    res = [xTop - (x[0]+0.5*dz*(-1-(u+SX.fmax(xTop/h, 0))/KFun(xTop, p, pars))),
           xBot - x[-1]]
    # res = [xTop - (x[0]+0.5*dz*(-1-(u)/KFun(xTop, p, pars))),
    #        xBot - x[-1]]
    return vertcat(*res)
...
# irrigation[0: int(22/ratio_t)] = -(0.001 / (pi * r * r) / (22 * 60))
# irrigation[int(59/ratio_t): int(87/ratio_t)] = -(0.001 / (pi * r * r) / (27 * 60))
# irrigation[int(161/ratio_t): int(189/ratio_t)] = -(0.001 / (pi * r * r) / (27 * 60))
# irrigation[int(248/ratio_t): int(276/ratio_t)] = -(0.001 / (pi * r * r) / (27 * 60))
# irrigation[int(335/ratio_t): int(361/ratio_t)] = -(0.001 / (pi * r * r) / (25 * 60))
# irrigation[int(1590/ratio_t): int(1656/ratio_t)] = -(0.001 / (pi * r * r) / (65 * 60))
...
p_min = np.ones(5)*0.5     # Parameter's lower bound
p_min[1] = 0.365/pars['thetaS']  # Its a value smaller than 1
p_min[2] = 0.5
p_max = np.ones(5)*3         # Parameter's upper bound
p_max[1] = 3
...
x_min = np.ones(32)*(-inf)         # Lower bound on states
x_max = np.zeros(32)      # Upper bound on states

y_init = np.array([10.1, 8.4, 8.6, 10.0])  # 1444 case
x_init, x_init4 = ini_state(y_init, p_init, pars)       # Initial state guess
xi_min = x_init*1.05      # Initial condition/state [1]
xi_max = x_init*0.95      # Initial condition/state. Same value as [1]
...
h_exp = np.zeros((1444, 4))  # four sensors
theta_exp = np.zeros((1444, 4))
...
# with open('Data/exp_data_5.dat', 'r') as f:
...
alg = getALG(x, z, u, p, h)                                     # Symbolic alg
...
for i in range(len(timeList)-1):
    ...
    Ik = I(x0=G, p=vertcat(irrigation[i], 1., 1., 1., 1., 1.))  # integrator with initial state G, and input U[k]
    G = Ik['xf']  # Assign the finial state to the initial state
    G_np = Ik['xf'].full().ravel()
    GL[i+1] = G_np

thetaL = thetaFun1(GL, [1,1,1,1,1], pars)
theta_avg = mtimes(thetaL, CMatrix.T)



plt.figure()
plt.plot(timeList_original/h*ratio_t, y_exp[:, 0], 'b:', label=r'$theta_1$ numpy')
plt.plot(timeList/h*ratio_t, theta_avg[:, 0], 'y-', label=r'$theta_1$ casadi')
...
plt.figure()
plt.plot(timeList_original/h*ratio_t, y_exp[:, 1], 'b:', label=r'$theta_2$ numpy')
plt.plot(timeList/h*ratio_t, theta_avg[:, 1], 'y-', label=r'$theta_2$ casadi')
...
plt.figure()
plt.plot(timeList_original/h*ratio_t, y_exp[:, 2], 'b:', label=r'$theta_3$ numpy')
plt.plot(timeList/h*ratio_t, theta_avg[:, 2], 'y-', label=r'$theta_3$ casadi')
...
plt.figure()
plt.plot(timeList_original/h*ratio_t, y_exp[:, 3], 'b:', label=r'$theta_4$ numpy')
plt.plot(timeList/h*ratio_t, theta_avg[:, 3], 'y-', label=r'$theta_4$ casadi')
