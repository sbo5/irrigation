...
def ini_state(y_init, p, pars):
    psiIni = hFun(y_init, p, pars)
...
for i in range(len(timeList)):
    if i == 0:
        ...
        pass
    Ik = I(x0=G,  p=vertcat(U[i], P))  # integrator with initial state G, and input U[k]
    # if (i*ratio_t) % 3 == 0:
    #     j = int(i*ratio_t)
