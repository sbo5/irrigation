...
def LoamOpt():
    pars = {}
    pars['thetaR'] = 0.078*0.995809
    pars['thetaS'] = 0.43*0.848928
    pars['alpha'] = 0.036*100*0.716791
    pars['n'] = 1.56*0.929861
    pars['m'] = 1 - 1 / pars['n']
    pars['Ks'] = 1.04/100/3600*1.98193
    pars['neta'] = 0.5
    pars['Ss'] = 0.00001
    pars['mini'] = 1.e-20
    return pars
...
def psiTopFun(hTop0, p, psi, qTop, dz):
    F = psi[0] + 0.5*dz*(-(qTop+max(0, hTop0/dt))/KFun(hTop0, p)-1) - hTop0
...
for i in range(0, len(irrigation)):  # 1st node is constant for 1st temporal element. The last node is the end of the last temporal element.
    ...
    if i in range(0, int(22/ratio_t)):
        irrigation[i] = -(0.0001 / (pi * 0.22 * 0.22) / (22 * 60))
    elif i in range(int(59/ratio_t), int(87/ratio_t)):
        irrigation[i] = -(0.0001 / (pi * 0.22 * 0.22) / (27 * 60))
        ...
        irrigation[i] = -(0.0001 / (pi * 0.22 * 0.22) / (25 * 60))
    elif i in range(int(1590/ratio_t), int(1656/ratio_t)):
        irrigation[i] = -(0.0004 / (pi * 0.22 * 0.22) / (65 * 60))
    else:
        irrigation[i] = 0
    # # 2683 case
