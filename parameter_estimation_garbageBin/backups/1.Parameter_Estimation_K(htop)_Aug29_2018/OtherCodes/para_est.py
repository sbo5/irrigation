...
def Loam():
    ...
    pars['neta'] = 0.5
    pars['Ss'] = 0.0001  # same as RichardsFoam2
    return pars
...
def capillary_capacity(h, p, s=S):
    ...
    Se = if_else(h>=0., 1., (1+(-h*p[3]*pars['alpha']+mini)**(p[4]*pars['n'])+mini)**(-(1-1/(p[4]*pars['n']+mini))))
...
for i in range(interval):
    if i == 0:
        G, GIni = ini_state(K)  # Initial state
    else:
        pass
