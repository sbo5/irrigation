...
def KFun(psi,pars):
    he = -0.05  # [m]
    Sc = ((1+abs(he*pars['alpha']+mini)**pars['n']+mini)**(-pars['m']))
    Se = if_else(psi>=he, 1., ((1+abs(psi*pars['alpha']+mini)**pars['n']+mini)**(-pars['m']))/Sc)
    K = if_else(Se>=1, pars['Ks'], pars['Ks']*(Se+mini)**pars['neta']*((1-((1-(Se*Sc+mini)**(1/(pars['m']+mini)))+mini)**pars['m']+mini)/(1-((1-(Sc+mini)**(1/(pars['m']+mini)))+mini)**pars['m']+mini))**2)
    K = K.full().ravel()
    return K
