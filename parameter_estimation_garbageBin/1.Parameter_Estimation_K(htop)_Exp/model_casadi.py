...
# y_init = np.array([10.1, 8.4, 8.6, 10.0])  # 336/1444/4361 case
...
# Simulation only
# ----------------------------------------------------------------------------------------------------------------------
# G, G4 = ini_state(y_init, [1., 1., 1., 1., 1.], pars)
...
#     Ik = I(x0=G, z0=Z, p=vertcat(irrigation[i], [1., 1., 1., 1., 1.]))  # integrator with initial state G, and input U[k]
...
G, G4 = ini_state(y_init, [3.46666665e-06/pars['Ks'],  3.98874113e-01/pars['thetaS'], 6.24000002e-02/pars['thetaR'], 2.88000000e+00/pars['alpha'],
                           1.49342952e+00/pars['n']], pars)
GL = np.zeros((len(timeList), numberOfNodes))
GL[0] = G
Z = [G[0], G[-1]]
...
for i in range(len(timeList)-1):
    print('From', i, ' min(s), to ', i + 1, ' min(s)')
    Ik = I(x0=G, z0=Z, p=vertcat(irrigation[i], [3.46666665e-06/pars['Ks'],  3.98874113e-01/pars['thetaS'], 6.24000002e-02/pars['thetaR'], 2.88000000e+00/pars['alpha'],
                           1.49342952e+00/pars['n']]))  # integrator with initial state G, and input U[k]
    G = Ik['xf']  # Assign the finial state to the initial state
    G_np = Ik['xf'].full().ravel()
    GL[i+1] = G_np
    Z = Ik['zf']
thetaL_opt = thetaFun1(GL, [3.46666665e-06/pars['Ks'],  3.98874113e-01/pars['thetaS'], 6.24000002e-02/pars['thetaR'], 2.88000000e+00/pars['alpha'],
                           1.49342952e+00/pars['n']], pars)
...
# np.savetxt('sim_results_336', theta_i)
