...
def LoamOpt():
    pars = {}
    pars['thetaR'] = 6.24000002e-02
    pars['thetaS'] = 3.98874113e-01
    pars['alpha'] = 2.88000000e+00
    pars['n'] = 1.49342952e+00
    pars['m'] = 1 - 1 / pars['n']
    pars['Ks'] = 3.46666665e-06
    pars['neta'] = 0.5
    pars['Ss'] = 0.00001
    pars['mini'] = 1.e-20
    return pars
...
# h_e = np.zeros((1444, 4))  # four sensors
# theta_e = np.zeros((1444, 4))
...
for i in range(0, len(irrigation)):  # 1st node is constant for 1st temporal element. The last node is the end of the last temporal element.
    ...
    if i in range(1, int(23/ratio_t)):
        ...
        irrigation[i] = -(0.001 / (pi * r * r) / (27 * 60))
    elif i in range(int(336/ratio_t), int(362/ratio_t)):
        irrigation[i] = -(0.001 / (pi * r * r) / (25 * 60))
    elif i in range(int(1589/ratio_t), int(1655/ratio_t)):
...
plt.figure()
plt.plot(timeList_original/dt*ratio_t, theta_e[:, 1], 'b:', label=r'$\theta_{2, exp}$')
plt.plot(timeList/dt*ratio_t, theta_i[:, 1], 'y-', label=r'$\theta_{2, ini}$')
plt.plot(timeList/dt*ratio_t, theta_opt[:, 1], 'r-', label=r'$\theta_{2, est}$')
plt.xlabel('Time, t (min)')
plt.ylabel('Water content (%)')
plt.legend(loc='best')
plt.show()
...
plt.figure()
plt.plot(timeList_original/dt*ratio_t, theta_e[:, 2], 'b:', label=r'$\theta_{3, exp}$')
plt.plot(timeList/dt*ratio_t, theta_i[:, 2], 'y-', label=r'$\theta_{3, ini}$')
plt.plot(timeList/dt*ratio_t, theta_opt[:, 2], 'r-', label=r'$\theta_{3, est}$')
plt.xlabel('Time, t (min)')
plt.ylabel('Water content (%)')
plt.legend(loc='best')
plt.show()
...
plt.figure()
plt.plot(timeList_original/dt*ratio_t, theta_e[:, 3], 'b:', label=r'$\theta_{4, exp}$')
plt.plot(timeList/dt*ratio_t, theta_i[:, 3], 'y-', label=r'$\theta_{4, ini}$')
plt.plot(timeList/dt*ratio_t, theta_opt[:, 3], 'r-', label=r'$\theta_{4, est}$')
